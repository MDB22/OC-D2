...
\begin{table}[!ht]
	...
	\begin{tabularx}{\textwidth}{|Y|Y|c|}
		...
		\hline
		\MULTROW{Loss of GPS link to UAV (Hover)}[*][4] & Switch to PosHold mode until a GPS fix is acquired; activate flight termination if backup position sensors estimate UAV has crossed a Geofence boundary & \MULTROW{Low}[*][4] \\
		\hline
		\MULTROW{Loss of GPS link to UAV (Fixed wing)}[*][4] & Switch to Circle mode until a GPS fix is acquired; activate flight termination if backup position sensors estimate UAV has crossed a Geofence boundary & \MULTROW{Low}[*][4]\\
		\hline
